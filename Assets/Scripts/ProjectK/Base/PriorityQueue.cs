...
namespace ProjectK.Base
{
        ...
        where T : class, IPriorityQueueNode
    {
        ...
        {
            #if DEBUG
            if(node.QueueIndex < 0 || node.QueueIndex >= _capacity)
            {
        ...
        private void Resize(int newCapacity)
        {
            ...
            int highestIndexToCopy = Math.Min(newCapacity - 1, _numNodes);
        ...
        public bool IsValidQueue()
        {
            for(int i = 1; i < _capacity; i++)
            {
                if(_nodes[i] != null)
                {
                    ...
                    if(childLeftIndex < _capacity && _nodes[childLeftIndex] != null && HasHigherPriority(_nodes[childLeftIndex], _nodes[i]))
                        return false;

                    int childRightIndex = childLeftIndex + 1;
                    if(childRightIndex < _capacity && _nodes[childRightIndex] != null && HasHigherPriority(_nodes[childRightIndex], _nodes[i]))
                        return false;
                }
            }
            return true;
